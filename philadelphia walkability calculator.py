...
import os
os.environ['HTTP_PROXY'] = 'http://c1proxy.frb.org:8080'
os.environ['HTTPS_PROXY'] = 'http://c1proxy.frb.org:8080'
...
# Does location matter? (Nearby projects method)
lines.append("\n-- Does Location Matter? Geographic Analysis ----------")

# For each project, find average score of projects within 1 mile
def get_nearby_avg(idx):
    """Get average walkability of nearby projects (within 1 mile)"""
    project = output.iloc[idx]
    
    # Calculate distance to all other projects
    distances = output.apply(
        lambda row: ((row['lat'] - project['lat'])**2 + 
                     (row['lon'] - project['lon'])**2)**0.5 * 69,
        axis=1
    )
    
    # Find nearby projects (within 1 mile, excluding self)
    nearby = (distances > 0) & (distances <= 1.0)
    
    if nearby.sum() == 0:
        return None
    
    return output.loc[nearby, 'walkability_total'].mean()

output['nearby_avg'] = [get_nearby_avg(i) for i in range(len(output))]

# Remove projects with no neighbors
has_neighbors = output['nearby_avg'].notna()

if has_neighbors.sum() > 0:
    location_corr = output.loc[has_neighbors, ['nearby_avg', 'walkability_total']].corr()
    lines.append(location_corr.to_string())
    
    corr_value = output.loc[has_neighbors, 'nearby_avg'].corr(output.loc[has_neighbors, 'walkability_total'])
    lines.append(f"\nCorrelation: {corr_value:.4f}")
    lines.append(f"Projects with neighbors within 1 mile: {has_neighbors.sum()} of {len(output)}")
    
    if corr_value > 0.5:
        lines.append("→ YES, LOCATION MATTERS: Nearby projects have similar scores")
    elif corr_value > 0.3:
        lines.append("→ LOCATION SOMEWHAT MATTERS: Moderate geographic clustering")
    else:
        lines.append("→ NO, LOCATION DOESN'T MATTER: Scores vary randomly across geography")
else:
    lines.append("Note: Projects are too spread out - cannot assess geographic clustering")
